...
public class SavableInventory extends SimpleInventory {
	...
	public SavableInventory(ScreenHandlerType<?> type, Identifier id, Text title) {
		...
		this.title = Objects.requireNonNullElse(title, LiteralText.EMPTY);
	...
	protected NbtCompound save() {
		...
		data.putString(TYPE_KEY, Registry.SCREEN_HANDLER.getId(type).toString());
		...
		Inventories.writeNbt(data, ((SimpleInventoryAccessor)this).getStacks());
		return data;
	}

	protected static SavableInventory load(NbtCompound data) {
		var type = Objects.requireNonNull(Registry.SCREEN_HANDLER.get(new Identifier(data.getString(TYPE_KEY))));
		var id = new Identifier(data.getString(ID_KEY));
		var title = Objects.requireNonNullElse(Text.Serializer.fromJson(data.getString(TITLE_KEY)), LiteralText.EMPTY);
		...
		inv.checker = checker;
		Inventories.readNbt(data, ((SimpleInventoryAccessor)inv).getStacks());
		return inv;
	}
